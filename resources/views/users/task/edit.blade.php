    ...
    <div class="flex justify-center">
        <div class="w-full max-w-lg">
            <div class="bg-white shadow-md rounded-lg">
                ...
                <div class="px-6 py-4">
                    <form method="POST" action="{{ route('task.update', $todo->todos_id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="task" name="task" value="{{ old('task', $todo->todo) }}" required
                        ...
                        <div class="mb-4">
                            ...
                            <select id="status" name="status" required
                                ...
                                <option value="0" {{ old('status', $todo->is_completed) == '0' ? 'selected' : '' }}>Pending</option>
                                <option value="1" {{ old('status', $todo->is_completed) == '1' ? 'selected' : '' }}>Completed</option>
                            </select>
                            @error('status')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
